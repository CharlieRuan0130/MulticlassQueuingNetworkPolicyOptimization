...
def run_weights(network_id, weights_set, policy, scaler, episode_length):

    # for n-classes, it is empty initialization regardless
    ...
    res.extend(ray.get([simulators[i].policy_performance_sixclasses.remote(network_id, scaler_id, initial_state_0, i, episode_length=episode_length)
...
def run_policy(network_id, policy, scaler, logger, gamma,
    ...
    for j in range(actors_per_run):
        accum_res.extend(ray.get([simulators[i].run_episode_alg3.remote(network_id, scaler_id, time_steps, cycles_num,
...
def add_disc_sum_rew(trajectories, policy, network, gamma, lam, scaler, iteration, no_AMP, sample_size):
    """
...
def main(network_id, num_policy_iterations, gamma, lam, kl_targ, batch_size, hid1_mult, episode_duration, cycles_num,
         clipping_parameter, skipping_steps, initial_state_procedure, no_AMP, sample_size):
    """
    # Main training loop
    ...
    trajectories, total_steps, action_optimal_sum, total_zero_steps, action_distr = \
        policy.run_episode_alg3(ray.get(network_id), scaler, episode_duration, cycles_num, skipping_steps, initial_states_set[0],
                           rpp=True)
    ...
    while iteration < num_policy_iterations:
        ...
        observes, disc_sum_rew_norm = add_disc_sum_rew(trajectories, policy, ray.get(network_id), gamma, lam, scaler, iteration, no_AMP, sample_size)  # calculate values from data
    ...
    performance_evolution_all, ci_all = run_weights(network_id, weights_set, policy, scaler, episode_length = 5*10**6)
    # performance_evolution_all, ci_all = run_weights(network_id, weights_set, policy, scaler, episode_length = 1000)
...
if __name__ == "__main__":
    ...
    start_time = datetime.datetime.now()
    network = pn.ProcessingNetwork.from_name('6-classes') # queuing network declaration
    end_time = datetime.datetime.now()
    time_policy = end_time - start_time
    ...
    parser.add_argument('--sample_size', type=int, help='Number of next states to sample when calculating AMP',
                        default = 5)
